...
class RiskyObject(nn.Module):
    ...
    def forward(self, x, y, toa, flow, hidden_in=None, testing=False):
        ...
        h = h.to(x.device)
        h_all_in = [{} for _ in x.shape[0]]
        h_all_out = [{} for _ in x.shape[0]]

        # hidden representation for secondary gru
        h_all_in_cor = [{} for _ in x.shape[0]]
        h_all_out_cor = [{} for _ in x.shape[0]]
        # h_all_in_flow = {}
        # h_all_out_flow = {}

        all_outputs = [[] for _ in x.shape[0]]
        all_labels = [[] for _ in x.shape[0]]

        for b in range(x.shape[0]):
            for t in range(x.size(1)):
                # projecting to a lower dimensional space
                inp = flow[:, t]  # B x 31 x 2048

                # Flow----------------
                x_val = self.phi_x(inp)  # B x 31 x 256  #rgb_d
                img_embed = x_val[:, 0, :].unsqueeze(1)  # B x 1 x 256
                img_embed = img_embed.repeat(1, 30, 1)  # B x 30 x 256
                obj_embed = x_val[:, 1:, :]   # B x 30 x 256
                ...
                for bbox in range(30):
                    if y[0][t][bbox][0] == 0:  # ignore if there is no bounding box TODO: here padding
                        continue
                    ...
                    if track_id in h_all_in:
                        ...
                        h_in = Variable(torch.zeros(self.n_layers, x.size(0), self.h_dim))  # orig TODO: hidden_in like dsta
            ...
            all_outputs[b].append(frame_outputs)
            all_labels[b].append(frame_labels)
            h_all_in = {}
